...
def test_simple_plaza(process_strategy, shortest_path_strategy):
    result_plaza = utils.process_plaza('helvetiaplatz', 4533221, process_strategy, shortest_path_strategy)
    assert result_plaza
...
def test_complicated_plaza(process_strategy, shortest_path_strategy):
    result_plaza = utils.process_plaza('bahnhofplatz_bern', 5117701, process_strategy, shortest_path_strategy)
    assert result_plaza
...
def test_obstructed_plaza(process_strategy, shortest_path_strategy):
    """ plazas obstructed by buildings should be discarded """
    result_plaza = utils.process_plaza('europaallee', 182055194, process_strategy, shortest_path_strategy)
    assert not result_plaza  # plaza should be discarded


def test_almost_obstructed_plaza(process_strategy, shortest_path_strategy):
    """ this plaza is almost completely obstructed, but a tiny area remains"""
    result_plaza = utils.process_plaza('zuerich_hb', 6619147, process_strategy, shortest_path_strategy)
    assert not result_plaza  # plaza should be discarded


def test_entry_points(process_strategy, shortest_path_strategy):
    """ Entry points on Kreuzplatz seem to be wrong """
    result_plaza = utils.process_plaza('kreuzplatz', 5541230, process_strategy, shortest_path_strategy)
...
def test_entry_lines(process_strategy, shortest_path_strategy):
    result_plaza = utils.process_plaza('helvetiaplatz', 39429064, process_strategy, shortest_path_strategy)
    assert result_plaza
...
def test_bahnhofstrasse(process_strategy, shortest_path_strategy):
    result_plaza = utils.process_plaza('bahnhofstrasse', 27405455, process_strategy, shortest_path_strategy)
...
def test_entry_points_with_cutout_polygon(process_strategy, shortest_path_strategy):
    result_plaza = utils.process_plaza('zuerich_hb', 6605179, process_strategy, shortest_path_strategy)
    assert result_plaza
